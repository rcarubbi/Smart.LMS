...
(function($) {
    ...
    function _toggleTypeFocus($input) {
        ...
        $input.closest("label").hover(function() {
                ...
                var isDisabled =
                    $i.prop("disabled"); // hack because the _addFormGroupFocus() wasn't identifying the property on chrome
        ...
        "input": function(selector) {
            $((selector) ? selector : this.options.inputElements)
                ...
                .each(function() {
                    ...
                    var $formGroup =
                        $input.closest(
                            ".form-group"); // note that form-group may be grandparent in the case of an input-group
                    if ($formGroup.length === 0 && $input.attr("type") !== "hidden" && !$input.attr("hidden")) {
                        $input.wrap("<div class='form-group'></div>");
                        $formGroup =
                            $input.closest(
                                ".form-group"); // find node after attached (otherwise additional attachments don't work)
                    }
                    ...
                    $.each(legacySizes,
                        function(legacySize, standardSize) {
